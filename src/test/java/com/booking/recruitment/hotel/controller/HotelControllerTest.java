...
class HotelControllerTest {
  ...
  @Test
  @DisplayName("When hotel by id is requested, it should return the same")
  ...
  @Test
  @DisplayName("When hotel is deleted by id should return success response")
  void deleteHotel() throws Exception {
    mockMvc
            .perform(get("/hotel/delete/1"))
            .andExpect(status().is2xxSuccessful());
    ...
    //Should have same number of entries as in DB
    mockMvc
            .perform(get("/hotel"))
            .andExpect(status().is2xxSuccessful())
            .andExpect(jsonPath("$", hasSize((int) repository.count())));
